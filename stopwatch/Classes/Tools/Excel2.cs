    ...
    public class ExcelEditor : IDisposable
    {
        ...
        public int CurrentSheet
        {
            ...
            set
            {
                ...
                {
                    if (CurrentSheet >= 1)
                        File.WriteAllText(dir + "xl\\worksheets\\sheet" + CurrentSheet + ".xml", sheet_data.ToStr());
                    if ((_CurrentSheet = value) > 0)
                    {
                        sheet_data = SheetData.Generate(File.ReadAllText(dir + "xl\\worksheets\\sheet" + CurrentSheet + ".xml").Replace("\r\n", ""));
                    }
                }
            }
        }
        ...
        {
            public string[] rows;
            public static SheetData Generate(string str)
            {
                var res = new SheetData();
                res.rows = str.Split(new string[] { "<row " }, StringSplitOptions.None);
                return res;
            }
            public string ToStr()
            {
                ...
                {
                    sb.Append((i > 0 ? "<row " : "") + rows[i]);
                }
                return sb.ToString();
            }
            public index IndexOf(string str, index start = null)
            {
                start = start ?? new index { row = 0, col = 0 };
                if (start.row < 0)
                    start = new index { row = 0, col = 0 };
                var col = start.col;
                for (int row = start.row; row < rows.Length; row++)
                {
                    var p = rows[row].IndexOf(str, col);
                    if (p >= 0)
                        return new index { row = row, col = p };
                    col = 0;
                }
                return null;
            }
            public class index
            {
                ...
                internal index Add(int v)
                {
                    col += v;
                    return this;
                }
                public static index operator +(index i, int shift)
                {
                    return new index { row = i.row, col = i.col + shift };
                }
                public static index operator -(index i, int shift)
                {
                    return new index { row = i.row, col = i.col - shift };
                ...
                {
                    return new index { row = a.row, col = a.col - b.col };
                }
            ...
            internal string Substring(index i, int length)
            {
                return rows[i.row].Substring(i.col, length);
            }
            internal string Substring(index i, index length)
            {
                return rows[i.row].Substring(i.col, length.col);
            }

            internal SheetData Replace(string v1, string v2, int count)
            {
                for (int i = 0; i < rows.Length; i++)
                {
                    if (count == 0) break;
                    var tmp = rows[i].Replace(v1, v2);
                    if (rows[i] != tmp)
                    {
                        count--;
                        rows[i] = tmp;
            ...
            internal SheetData Remove(index i, index count)
            {
                rows[i.row] = rows[i.row].Remove(i.col, count.col);
                return this;
            }

            internal SheetData Insert(index p2, string v)
            {
                rows[p2.row] = rows[p2.row].Insert(p2.col, v);
                return this;
            }

            internal SheetData Replace(index p1, index p2, string v)
            {
                rows[p1.row] = rows[p1.row].Substring(0, p1.col) + v + rows[p1.row].Substring(p2.col);
                return this;
            }
        }
        internal SheetData sheet_data;
        ...
        public void SetCellValue(string cell, string value, int hint_row = -1)
        {
            try
            {
                ...
                var p1 = sheet_data.IndexOf("r=\"" + cell + "\"", new SheetData.index { row = hint_row });
                if (p1 == null)
        ...
        {
            column++;
            sheet_data = sheet_data.Replace("</cols>", "<col min=\"" + column + "\" max=\"" + column + "\" width=\"" + width + "\"  customWidth=\"1\"/></cols>", 1);
        }
        public void AddHyperLink(string cell, string link)
        {
            ...
            sheet_data = sheet_data.Replace("</hyperlinks>", "<hyperlink ref=\"" + cell + "\" r:id=\"rId" + id + "\"/></hyperlinks>", 1);
        }
        ...
        public string GetCellStyle(string cell)
        {
            ...
            if (p1 == null) return "";
            var p2 = sheet_data.IndexOf("s=\"", p1).Add(3);
            var p3 = sheet_data.IndexOf("\"", p2 + 1);
            var p4 = sheet_data.IndexOf(">", p1);
            if (p4 < p2)
                return "";
            else
                return sheet_data.Substring(p2, p3.col - p2.col);
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        public string GetCellValue(string cell, int hint_row = -1)
        {
            ...
            if (p4 < p3)
            {
                p2 = sheet_data.IndexOf("<v>", p1).Add(3);
